...
import (
	"bufio"
	"bypass-detection/config"
	"bypass-detection/logger"
	"bytes"
	"crypto/sha1"
	"encoding/hex"
	"fmt"
...
func main() {
	...
	if err != nil {
		logger.Logger.Fatalf("Failed to bind %s, error: %s", cfg.Server.Address, err.Error())
	}
	logger.Logger.Infof("Listening for connections on %s", cfg.Server.Address)
